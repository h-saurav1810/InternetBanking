<?php
	...
	if (isset($_POST["submit"]))
	{
		...
		$sql = "SELECT accountno FROM institution WHERE institution = '$beneficiary'";
		$query = $con->query($sql);
		...
		if ($pin == $checkpin)
		{
			if($currentBalance > $amount) {
				...
				header("Location: payment_successful.php?account=$account&amount=$amount&receivingaccount=$receivingaccount&paybills=1");
			} else {
...
<li><a href="homepage2.php?account=<?php echo $account; ?>">Home</a></li>
    ...
    <form class='card_form' action="" method="post">
        <div class='container'>
        <div class='card'>
        <h1 style="font-size:22px" >Pay Bills  </h1>
        <br>
        <h1 style="font-size:14px" >Pay any pending bills easily and quickly using Horizon JomPayee </h1>
        <br>
       
        <label class="input_label">Beneficiary</label><br>
        <select class='input_field' name="beneficiary" placeholder="">
			<?php
				$sel = "SELECT institution FROM institution";
				$query = $con->query($sel);
				while ($bill = $query->fetch_array())
				{
			?>
			<option value="<?php echo $bill[0]?>"><?php echo $bill[0]?></option>
			<?php
				}
			?>
		</select>
