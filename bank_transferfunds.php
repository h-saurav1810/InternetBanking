	...
	*/	
		...
		if ($pin == $checkpin)
		{
			if($currentBalance > $amount) {
				...
				header("Location: payment_successful.php?account=$account&amount=$amount&receivingaccount=$receivingaccount&paybills=0");
			} else {
...
<ul>
    ...
    <form class='card_form' action="" method="post">
        <div class='container'>
        <div class='card'>
        <h1 style="font-size:22px" >Transfer Funds  </h1>
        <br>
        <h1 style="font-size:14px" >Complete a bank to bank transfer by entering the details </h1> 
        <br>
        <br>

        <label class="input_label">Bank Account </label><br>
        <input class='input_field' type="number" name="receiving_account" placeholder="XXXX-XXXX-XXXX">
