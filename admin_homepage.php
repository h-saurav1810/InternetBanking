...
<div class="logo">
	...
	<table width="50%" border='1'>
		...
		<?php
			...
			if ($constatus == 1)
			{
				$sel = "SELECT customer.accountno, customer.firstname, customer.lastname, customer.email, customer.username, account.balance, account.blocked FROM customer INNER JOIN account ON customer.accountno = account.accountno";
				$query = $con->query($sel);
		...
		<tr>
			...
			<td><input type="submit" value="Unblock" name="unblock" formaction="admin_blockunblock.php?account=<?php echo $row[0];?>"/></td>
			<td><input type="submit" value="Edit" name="edit" formaction="admin_useredit.php?account=<?php echo $row[0];?>"/></td>
		</tr>
		<?php
				}
			}
