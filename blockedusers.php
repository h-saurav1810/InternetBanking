...
<body>
	...
	<table cellpadding="5px" width="50%" border='1'>
		...
		<?php
			...
			if ($constatus == 1)
			{
				$sel = "SELECT customer.accountno, customer.firstname, customer.lastname, customer.email, customer.username, account.balance, account.blocked FROM customer INNER JOIN account ON customer.accountno = account.accountno WHERE account.blocked = 1";
				$query = $con->query($sel);
